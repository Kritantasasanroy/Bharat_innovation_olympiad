 ...
 *
 * `{ as: "global" }` is required here: portal-api composes each partner route
 * file as its own Elysia instance and `.use()`s them all into `app.ts`'s root
 * instance (see `partner-application.routes.ts` etc.). Elysia's `onError`
 * defaults to "local" scope, which only catches errors thrown by handlers
 * defined directly on the SAME instance — it would never see an error thrown
 * inside a sibling `.use()`'d plugin's handler without this.
 */
