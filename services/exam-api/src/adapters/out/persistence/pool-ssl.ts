 ...
 * table. `services/admin-api` hit exactly this on dev: every query 500'd while
 * `psql` against the identical URL worked, because libpq negotiates TLS on its
 * own and node-postgres does not. This client is the same shape, so it carries
 * the same fix rather than waiting to be found the same way.
 *
