...
- **Detector tuning:** `TinyFaceDetectorOptions` uses `inputSize: 512, scoreThreshold: 0.3` (both `runDetection` and `captureDescriptor` share these constants) instead of face-api.js's defaults (`416` / `0.5`). The defaults are tuned for cleaner, higher-res input than a typical laptop webcam under normal indoor lighting provides — with the original 320×240 camera resolution and default threshold, the detector was discarding real, clearly-visible faces as below-confidence, surfacing as false `NO_FACE` events. Since face-count, gaze estimation, and multi-face detection all run against the same `detectAllFaces()` call, this one fix improves reliability across all three, not just the face-presence check.
