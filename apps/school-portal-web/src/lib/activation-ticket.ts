 ...
 * coordinator confirms their email) back to `/activate` (where they fill in
 * the rest and submit) — the verify-first step.
 *
...
const STORAGE_KEY = "bio.school.activationTicket";
...
export function setActivationTicket(ticket: string, email: string): void {
	...
	try {
		...
		// Private mode / storage disabled — the coordinator just re-verifies.
	}
}
