...
const TOKEN_STORAGE_KEY = "bio-partner-portal.access-token";

/**
 * The subset of the shared-JWT payload (`{ sub, email, role }`, issued by
 * `backend/src/auth/auth.controller.ts` and verified by portal-api's
 * `auth.plugin.ts`) this app reads client-side. This is a *display-only*
 * decode (no signature check) — the real verification happens once per
 * request on portal-api; a tampered token here just fails every API call.
 */
